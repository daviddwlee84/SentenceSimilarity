...
class BiMPM(nn.Module):
    def __init__(self, embedding_matrix, max_len, num_class=1, freeze_embed=False):
        ...
        self.l = 20
        self.hidden_size = 100  # number of perspective
        self.p = 0.1  # dropout rate

        # ----- Word Representation Layer -----
        self.word_emb = nn.Embedding.from_pretrained(
            embedding_matrix, freeze=freeze_embed)

        self.d = self.word_emb.embedding_dim
        ...
        self.pred_fc2 = nn.Linear(self.hidden_size * 2, num_class)
    ...
    def forward(self, q1, q2):
        ...
        def mp_matching_func(v1, v2, w):
            ...
            if len(v2.size()) == 3:
                ...
                v2 = w * \
                    torch.stack(
                        [torch.stack([v2] * seq_len, dim=1)] * self.l, dim=3)
        ...
        def mp_matching_func_pairwise(v1, v2, w):
            ...
            v1, v2 = w * torch.stack([v1] * self.l, dim=1), w * \
                torch.stack([v2] * self.l, dim=1)
        ...
        att_mean_h_fw = div_with_small_value(
            att_h_fw.sum(dim=2), att_fw.sum(dim=2, keepdim=True))
        att_mean_h_bw = div_with_small_value(
            att_h_bw.sum(dim=2), att_bw.sum(dim=2, keepdim=True))

        # (batch, seq_len2, hidden_size) / (batch, seq_len2, 1) -> (batch, seq_len2, hidden_size)
        att_mean_p_fw = div_with_small_value(att_p_fw.sum(
            dim=1), att_fw.sum(dim=1, keepdim=True).permute(0, 2, 1))
        att_mean_p_bw = div_with_small_value(att_p_bw.sum(
            dim=1), att_bw.sum(dim=1, keepdim=True).permute(0, 2, 1))
